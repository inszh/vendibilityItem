...
//    self.collectionView.bounces = NO;//将网格视图的下拉效果关闭
    ...
    if (self.calendarMonth.count&&self.resultBlock) {
        self.resultBlock(self.calendarMonth);
    }
    
    self.navigationItem.hidesBackButton = YES;
    
    UIButton*backButton=[UIButton buttonWithType:UIButtonTypeCustom];
    [backButton setTitle:@"编辑数据" forState:0];
    [backButton setTitleColor:[UIColor blackColor] forState:0];
    backButton.frame=CGRectMake(0, 0, 44, 44);
    [backButton setImageEdgeInsets:UIEdgeInsetsMake(0, -15, 0, 15)];
    [backButton addTargetBlock:^(NSInteger tag) {
        FLQDataViewController *dataVc=FLQDataViewController.new;
